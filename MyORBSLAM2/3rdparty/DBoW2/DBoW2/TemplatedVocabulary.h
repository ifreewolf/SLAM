  ...
  BowVector &v, FeatureVector &fv, int levelsup) const
{
    ...
    if(m_weighting == TF || m_weighting == TF_IDF)
    {
        ...
        for(fit = features.begin(); fit < features.end(); ++fit, ++i_feature)
        {
            ...
            NodeId nid;     // FeatureVector里的NodeId，用于加速搜索
            WordValue w;    // 叶子节点word对应的权重
            
            // 将当前描述子转化为word id，word weight节点所属的父节点id（这里的父节点不是叶子的上一层，它距离叶子深度为levelsup）
            // w is the idf value if TF_IDF, 1 if TF
            transform(*fit, id, w, &nid, levelsup);
            
            if(w > 0) // not stopped
            {
                // 如果word权重大于0，将其添加到BowVector和FeatureVector
                v.addWeight(id, w);
...
{ 
    ...
    // 取出vocabulary tree中node距离当前feature描述子距离最小的那个node的word id和weight
