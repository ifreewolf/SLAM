...
- [一、c++语法检查增强](#一c语法检查增强)
  - [全局变量监测增强](#全局变量监测增强)
  - [1.1 C++中所有的变量和函数都必须有类型](#11-c中所有的变量和函数都必须有类型)
  - [1.2 更严格的类型转换](#12-更严格的类型转换)
  - [1.3 struct 类型加强](#13-struct-类型加强)
  - [1.4 新增“bool”类型关键字](#14-新增bool类型关键字)
  - [1.5 三目运算符功能增强](#15-三目运算符功能增强)
- [二、C/C++中的const](#二cc中的const)
  - [2.1 C和C++中const的区别](#21-c和c中const的区别)
    - [C中的const](#c中的const)
    - [2.1.1 C++中的const](#211-c中的const)
    - [2.1.2 尽量以const替换`#define`](#212-尽量以const替换define)
    - [2.1.3 const 和 define区别](#213-const-和-define区别)
- [三、引用(reference)](#三引用reference)
  - [引用作用于数组](#引用作用于数组)
  - [函数中的引用](#函数中的引用)
    - [在函数参数中使用引用](#在函数参数中使用引用)
    - [在函数返回值中使用引用](#在函数返回值中使用引用)
    - [引用的本质](#引用的本质)
    - [指针引用](#指针引用)
    - [常量引用](#常量引用)
    - [常量的引用](#常量的引用)
- [四、宏函数与内联函数的比较](#四宏函数与内联函数的比较)
  - [4.1 类内部的内联函数](#41-类内部的内联函数)
  - [4.2 内联函数和编译器](#42-内联函数和编译器)
- [五、函数的默认参数](#五函数的默认参数)
  - [5.1 函数的占位参数](#51-函数的占位参数)
- [六、函数重载(overload)](#六函数重载overload)
  - [6.1 函数重载实现原理](#61-函数重载实现原理)
- [七、C和C++混合编程](#七c和c混合编程)
- [八、类和对象](#八类和对象)
  - [8.1 类的初始](#81-类的初始)
  - [将成员变量设置为private](#将成员变量设置为private)
  - [8.2 类的大小](#82-类的大小)
  - [8.3 在类里声明，类外定义 成员函数。](#83-在类里声明类外定义-成员函数)
  - [8.4 对象的构造和析构](#84-对象的构造和析构)
    - [8.4.1 初始化和清理](#841-初始化和清理)
    - [8.4.2 构造和析构的定义](#842-构造和析构的定义)
    - [8.4.3 构造函数的分类](#843-构造函数的分类)
      - [构造函数分类](#构造函数分类)
      - [8.4.3.1 构造函数调用](#8431-构造函数调用)
      - [8.4.3.2 拷贝构造函数](#8432-拷贝构造函数)
      - [8.4.3.3 构造函数的调用规则](#8433-构造函数的调用规则)
      - [8.4.3.4 深拷贝和浅拷贝](#8434-深拷贝和浅拷贝)
      - [8.4.3.5 初始化列表](#8435-初始化列表)
    - [8.4.4 explict关键字](#844-explict关键字)
    - [8.4.5 动态对象创建](#845-动态对象创建)
      - [8.4.5.1 对象创建](#8451-对象创建)
      - [8.4.5.2 new operator](#8452-new-operator)
    - [8.4.6 对象数组](#846-对象数组)
      - [8.4.6.1 用new申请对象指针](#8461-用new申请对象指针)
      - [8.4.6.2 delete不让释放void指针](#8462-delete不让释放void指针)
    - [8.4.7 静态成员](#847-静态成员)
      - [8.4.7.1 静态成员变量](#8471-静态成员变量)
      - [8.4.7.2 静态成员函数](#8472-静态成员函数)
      - [8.4.7.3 const静态成员属性](#8473-const静态成员属性)
      - [8.4.7.4 静态案例](#8474-静态案例)
    - [8.4.8 this指针](#848-this指针)
      - [8.4.8.1 this指针的注意点](#8481-this指针的注意点)
      - [8.4.8.2 this指针的使用](#8482-this指针的使用)
  - [8.5 const修饰成员函数](#85-const修饰成员函数)
    - [8.5.1 const修饰对象(叫常对象)](#851-const修饰对象叫常对象)
  - [8.6 友元](#86-友元)
    - [8.6.1 普通全局函数作为类的友元](#861-普通全局函数作为类的友元)
    - [8.6.2 类的成员函数作为另一个类的友元](#862-类的成员函数作为另一个类的友元)
    - [8.6.3 类作为另一个类的友元](#863-类作为另一个类的友元)
    - [8.6.3 友元案例](#863-友元案例)
    - [8.6.4 强化类的封装](#864-强化类的封装)
  - [8.7 运算符重载](#87-运算符重载)
    - [8.7.1 重载运算符的概述](#871-重载运算符的概述)
    - [8.7.2 运算符`<<`的重载](#872-运算符的重载)
    - [8.7.3 全局友元函数重载加法运算符](#873-全局友元函数重载加法运算符)
    - [8.7.4 成员函数重载加法运算符](#874-成员函数重载加法运算符)
    - [8.7.5 加加和减减运算符重载](#875-加加和减减运算符重载)
    - [8.7.6 指针运算符重载(\*,-\>)](#876-指针运算符重载-)
    - [8.7.7 重载赋值运算符](#877-重载赋值运算符)
    - [8.7.8 重载不等以及相等运算符](#878-重载不等以及相等运算符)
    - [8.7.9 函数调用符()的重载](#879-函数调用符的重载)
    - [8.7.10 不要重载 \&\& 和 ||](#8710-不要重载--和-)
    - [8.7.11 符号重载的总结](#8711-符号重载的总结)
    - [8.7.12 强化训练 -- 字符串类封装](#8712-强化训练----字符串类封装)
  - [8.8 继承和派生类](#88-继承和派生类)
    - [8.8.1 子类的内层布局](#881-子类的内层布局)
    - [8.8.2 构造和析构的顺序](#882-构造和析构的顺序)
      - [8.8.2.1 子类的构造和析构](#8821-子类的构造和析构)
      - [8.8.2.2 子类中有父类、对象成员的构造和析构的顺序](#8822-子类中有父类对象成员的构造和析构的顺序)
      - [8.8.2.3 详解子类中的构造](#8823-详解子类中的构造)
    - [8.8.3 继承中同名函数的处理](#883-继承中同名函数的处理)
      - [8.8.3.1 父类和子类同名成员变量处理](#8831-父类和子类同名成员变量处理)
    - [8.8.4 父类和子类同名成员函数处理](#884-父类和子类同名成员函数处理)
      - [8.8.4.1 案例一](#8841-案例一)
      - [8.8.4.2 案例二](#8842-案例二)
    - [8.8.5 父子类中静态成员同名分析](#885-父子类中静态成员同名分析)
      - [8.8.5.1 静态成员变量同名分析](#8851-静态成员变量同名分析)
    - [8.8.6 菱形继承](#886-菱形继承)
    - [8.8.7 虚继承](#887-虚继承)
  - [8.9 多态](#89-多态)
    - [8.9.1 使用基类指针、引用访问子类对象中的成员方法(虚函数)](#891-使用基类指针引用访问子类对象中的成员方法虚函数)
      - [8.9.1.1 为什么使用虚函数](#8911-为什么使用虚函数)
      - [8.9.1.2 虚函数的应用案例](#8912-虚函数的应用案例)
      - [8.9.1.3 虚析构](#8913-虚析构)
        - [8.9.1.3.1 虚析构的分析](#89131-虚析构的分析)
    - [8.9.2 抽象基类和纯虚函数(pure virtual function)](#892-抽象基类和纯虚函数pure-virtual-function)
      - [8.9.2.1 纯虚函数和抽象类的引用](#8921-纯虚函数和抽象类的引用)
      - [8.9.2.2 纯虚析构函数](#8922-纯虚析构函数)
      - [8.9.2.3 虚函数、纯虚函数、虚析构、纯虚析构](#8923-虚函数纯虚函数虚析构纯虚析构)
      - [8.9.2.4 重载、重写、重定义](#8924-重载重写重定义)
  - [8.10 函数模板](#810-函数模板)
    - [8.10.1 函数模板和普通函数的区别](#8101-函数模板和普通函数的区别)
      - [8.10.1.1 函数模板练习](#81011-函数模板练习)
    - [8.10.2 模板的局限性](#8102-模板的局限性)
  - [8.11 类模板](#811-类模板)
    - [8.11.1 类模板做函数参数](#8111-类模板做函数参数)
    - [8.11.2 类模板派生普通类](#8112-类模板派生普通类)
    - [8.11.3 类模板类内实现](#8113-类模板类内实现)
    - [8.11.4 类模板类外实现](#8114-类模板类外实现)
    - [8.11.5 类模板头文件和源文件分离问题\</b](#8115-类模板头文件和源文件分离问题b)
    - [8.11.6 类模板案例](#8116-类模板案例)
    - [8.11.7 类模板和友元](#8117-类模板和友元)
- [九、类型转换](#九类型转换)
  - [9.1 静态转换`static_cast`](#91-静态转换static_cast)
  - [9.2 动态转换`dynamic_cast`](#92-动态转换dynamic_cast)
  - [9.3 常量转换`const_cast`](#93-常量转换const_cast)
  - [9.4 重新解释转换`reinterpret_cast`](#94-重新解释转换reinterpret_cast)
- [十、C++异常](#十c异常)
  - [10.1 异常基本概念](#101-异常基本概念)
  - [10.2 栈解旋(unwinding)](#102-栈解旋unwinding)
  - [10.3 异常接口声明](#103-异常接口声明)
  - [10.4 异常的生命周期](#104-异常的生命周期)
  - [10.5 标准异常](#105-标准异常)
    - [10.5.1 标准库介绍](#1051-标准库介绍)
    - [10.5.2 编写自己的异常类](#1052-编写自己的异常类)
- [十一、cin拓展](#十一cin拓展)
- [C++扩展知识点](#c扩展知识点)
  - [iota函数](#iota函数)
>>>>>>> 3baf73d37b7772ddc0e73ffc39ac59a1ddc1ef64
...
### 8.9.1 使用基类指针、引用访问子类对象中的成员方法(虚函数)
...
### 8.9.2 抽象基类和纯虚函数(pure virtual function)
...
iota算法是一个函数模板，用于填充一个区间，以递增的方式给每个元素赋予一个值
