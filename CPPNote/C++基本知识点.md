...
- [四、类和对象](#四类和对象)
  ...
  - [类的大小](#类的大小)
  - [在类里声明，类外定义 成员函数。](#在类里声明类外定义-成员函数)
  - [对象的构造和析构](#对象的构造和析构)
    - [初始化和清理](#初始化和清理)
    - [构造和析构的定义](#构造和析构的定义)
    - [构造函数的分类](#构造函数的分类)
    ...
    - [对象数组](#对象数组)
      - [用new申请对象指针](#用new申请对象指针)
      - [delete不让释放void指针](#delete不让释放void指针)
    - [静态成员](#静态成员)
      - [静态成员变量](#静态成员变量)
      - [静态成员函数](#静态成员函数)
      - [const静态成员属性](#const静态成员属性)
      - [静态案例](#静态案例)
    - [this指针](#this指针)
      - [this指针的注意点](#this指针的注意点)
      - [this指针的使用](#this指针的使用)
  - [const修饰成员函数](#const修饰成员函数)
    - [const修饰对象(叫常对象)](#const修饰对象叫常对象)
  - [友元](#友元)
    - [普通全局函数作为类的友元](#普通全局函数作为类的友元)
    - [类的成员函数作为另一个类的友元](#类的成员函数作为另一个类的友元)
    - [类作为另一个类的友元](#类作为另一个类的友元)
    - [友元案例](#友元案例)
    - [强化类的封装](#强化类的封装)
  - [运算符重载](#运算符重载)
    - [重载运算符的概述](#重载运算符的概述)
    - [运算符`<<`的重载](#运算符的重载)
    - [全局友元函数重载加法运算符](#全局友元函数重载加法运算符)
    - [成员函数重载加法运算符](#成员函数重载加法运算符)
    - [加加和减减运算符重载](#加加和减减运算符重载)
    - [指针运算符重载(\*,-\>)](#指针运算符重载-)
    - [重载赋值运算符](#重载赋值运算符)
    - [重载不等以及相等运算符](#重载不等以及相等运算符)
    - [函数调用符()的重载](#函数调用符的重载)
    - [不要重载 \&\& 和 ||](#不要重载--和-)
    - [符号重载的总结](#符号重载的总结)
    - [强化训练 -- 字符串类封装](#强化训练----字符串类封装)
  - [五、继承和派生类](#五继承和派生类)
    - [5.1 子类的内层布局](#51-子类的内层布局)
    - [5.2 构造和析构的顺序](#52-构造和析构的顺序)
      - [5.2.1 子类的构造和析构](#521-子类的构造和析构)
      - [5.2.2 子类中有父类、对象成员的构造和析构的顺序](#522-子类中有父类对象成员的构造和析构的顺序)
      - [详解子类中的构造](#详解子类中的构造)
    - [5.3 继承中同名函数的处理](#53-继承中同名函数的处理)
    - [5.3.1 父类和子类同名成员变量处理](#531-父类和子类同名成员变量处理)
    - [5.3.2 父类和子类同名成员函数处理](#532-父类和子类同名成员函数处理)
      - [案例一](#案例一)
      - [案例二](#案例二)
    - [5.3.3 父子类中静态成员同名分析](#533-父子类中静态成员同名分析)
      - [5.3.3.1 静态成员变量同名分析](#5331-静态成员变量同名分析)
    - [5.3.4 菱形继承](#534-菱形继承)
    - [5.3.5 虚继承](#535-虚继承)
  - [多态](#多态)
    - [使用基类指针、引用访问子类对象中的成员方法(虚函数)](#使用基类指针引用访问子类对象中的成员方法虚函数)
      - [为什么使用虚函数](#为什么使用虚函数)
      - [虚函数的应用案例](#虚函数的应用案例)
      - [虚析构](#虚析构)
        - [虚析构的分析](#虚析构的分析)
    - [抽象基类和纯虚函数(pure virtual function)](#抽象基类和纯虚函数pure-virtual-function)
      - [纯虚函数和抽象类的引用](#纯虚函数和抽象类的引用)
      - [纯虚析构函数](#纯虚析构函数)
      - [虚函数、纯虚函数、虚析构、纯虚析构](#虚函数纯虚函数虚析构纯虚析构)
      - [重载、重写、重定义](#重载重写重定义)
  - [函数模板](#函数模板)
  ...
  - [类模板](#类模板)
    ...
    - [类模板类内实现](#类模板类内实现)
    - [类模板类外实现](#类模板类外实现)
    - [类模板头文件和源文件分离问题\</b](#类模板头文件和源文件分离问题b)
    - [类模板案例](#类模板案例)
    - [类模板和友元](#类模板和友元)
- [六、类型转换](#六类型转换)
  - [6.1 静态转换`static_cast`](#61-静态转换static_cast)
  - [6.2 动态转换`dynamic_cast`](#62-动态转换dynamic_cast)
  - [6.3 常量转换`const_cast`](#63-常量转换const_cast)
  - [6.4 重新解释转换`reinterpret_cast`](#64-重新解释转换reinterpret_cast)
- [七、C++异常](#七c异常)
  - [7.1 异常基本概念](#71-异常基本概念)
  - [7.2 栈解旋(unwinding)](#72-栈解旋unwinding)
  - [7.3 异常接口声明](#73-异常接口声明)
  - [7.4 异常的生命周期](#74-异常的生命周期)
  - [7.5 标准异常](#75-标准异常)
    - [7.5.1 标准库介绍](#751-标准库介绍)
    - [7.5.2 编写自己的异常类](#752-编写自己的异常类)
- [cin拓展](#cin拓展)
